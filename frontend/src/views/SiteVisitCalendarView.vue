...
onMounted(loadMonth)
// Reset the selected day to the 1st whenever the visible month changes —
// otherwise a stale selectedDate from a previous month would look inside
// `visits`, which is now scoped to the newly-loaded month and would never
// contain it, silently showing "no visits" for a date not even in view.
watch(month, () => {
  selectedDate.value = dateFor(1)
  loadMonth()
})
</script>
  ...
  <div class="space-y-5">
    ...
    <section v-else class="grid gap-5 xl:grid-cols-[1fr_320px]">
      <div class="rounded-2xl border border-border bg-surface overflow-hidden">
        ...
        <div class="grid grid-cols-7">
          <div
            ...
            :key="index"
            class="min-h-28 border-b border-r border-border p-2 cursor-pointer"
            :class="day && selectedDate === dateFor(day) ? 'bg-primary-soft/40' : ''"
            @click="day && (selectedDate = dateFor(day))"
          >
            <template v-if="day">
              <div class="flex items-center justify-between">
                <span
                  class="text-xs font-semibold"
                  :class="dateFor(day) === todayIso ? 'flex h-6 w-6 items-center justify-center rounded-full bg-primary text-primary-foreground' : 'text-muted'"
                >
                  {{ day }}
                </span>
              ...
              <button
                v-for="v in visitsFor(dateFor(day)).slice(0, 2)"
                :key="v.name"
              ...
              >
                <span class="block truncate">{{ v.customer_name }}</span>
              </button>
              <p v-if="visitsFor(dateFor(day)).length > 2" class="mt-1 text-[10px] text-muted">+{{ visitsFor(dateFor(day)).length - 2 }} more</p>
            </template>
          </div>
        </div>
      </div>
      ...
      <aside class="rounded-2xl border border-border bg-surface p-5 h-fit">
        ...
        <h2 class="mt-2 text-xl font-semibold text-foreground">{{ formatDate(selectedDate) }}</h2>

        <div v-if="selectedVisits.length" class="mt-5 space-y-3">
          <button v-for="v in selectedVisits" :key="v.name" class="w-full rounded-xl bg-surface-muted p-3 text-left" @click="router.push(`/site-visits/${v.name}`)">
            <div class="flex items-center justify-between gap-2"><p class="font-semibold text-foreground">{{ v.name }}</p><StatusBadge :status="v.status" /></div>
