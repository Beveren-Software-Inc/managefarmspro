...
def get_data(filters):
	...
	invoices = (
		...
		.select(
			...
			Plot.plot_name,
			Work.work_date,
			Work.work_type_name.as_("work_name"),
			Work.description,
			Work.total_cost,
			Work.customer,
		)
...
# 			"doctype": "File",
# 			"file_name": file_name,
# 			"file_url": f"/private/files/{file_name}",  # Save without content_hash
...
# 	# Update Work Doctype with the correct file URL (including the content_hash suffix)
# 	final_file_url = f"/private/files/Invoice_{plot_name}_{filters.get('start_date')}_{filters.get('end_date')}_{unique_suffix}{hash_suffix}.pdf"

# 	# Update the Sales Invoice with the correct file URL
# 	frappe.db.set_value("Sales Invoice", invoice_number, "pdf_invoice_link", final_file_url)

# 	# Update Work Doctype with the correct file URL
# 	for invoice in invoices:
# 		frappe.db.set_value("Work", invoice["work_id"], "pdf_invoice_link", final_file_url)

# 	# Construct full URL with site path
# 	site_url = frappe.utils.get_site_url(frappe.local.site)
# 	full_url = f"{site_url}{final_file_url}"
